...
class DataAccess {
    ...
    private func getBody<T: Encodable>(api: String, req: T) -> Request<T> {
        let body = Request(API_CRTS_KEY: "metc650e-ce68-48ea-a96e-6c554d91165",
                           API_SVC_ID: api,
                           USE_INTT_ID: ShareInstance.manager.USE_INTT_ID,
                           BIZ_NO: ShareInstance.manager.BIZ_NO,
                           USER_ID: ShareInstance.manager.USER_ID,
                           API_SEQ_NO: "",
                           LST_TRAN_YN: "Y",
                           REQ_DATA: req)
        return body
    ...
    private func request<T: Encodable>(api: String, body: T) -> URLRequest {
        var urlSt = "http://serpcms.dev.wecontent.co.kr/gw/MetcGateWay"
        var methodSt = "POST"
        if api == "mg" {
            urlSt = "http://sportal.dev.weplatform.co.kr:19990/MgGate?master_id=I_BIPLE_RECEIPT_G_1"
            methodSt = "GET"
        }
        ...
        if api != "mg" {
            let fullBody = getBody(api: api, req: body)
            let strQuery = queryString(body: fullBody)
